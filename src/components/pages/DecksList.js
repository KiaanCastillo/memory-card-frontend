...
const DecksList = () => {
  ...
  const getDecksByOwnerId = async (id) => {
    try {
      const response = await axios.get(
        `https://1dllzo95qa.execute-api.us-east-1.amazonaws.com/dev/getdeckbyownerid/${id}`
      );
  ...
  const deleteDeck = async (id) => {
    try {
      await axios.delete(
        `https://1dllzo95qa.execute-api.us-east-1.amazonaws.com/dev/deletedeckbyid/${id}`
      );
